...
#popularidad_df$actores <- strsplit(movies$actors, "\\|")

actors <- movies$actors

actorsPopularity <- movies$actorsPopularity

popularidad_df <- data.frame(actors, actorsPopularity)


popularidad_df$actorsPopularity <- lapply(popularidad_df$actorsPopularity, function(x) {
  x <- as.numeric(unlist(strsplit(x, "\\|")))
  x[is.na(x) | !is.finite(x) | !is.numeric(x)] <- NA
  x
})

popularidad_df$avg <- sapply(popularidad_df$actorsPopularity, mean, na.rm = TRUE)

movies$popularidad_avg <- popularidad_df$avg

summary(movies$popularidad_avg)

ggplot(movies, aes(x=popularidad_avg, y=revenue)) + 
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

```

Como se puede observar la popularidad de los actores no afecta directamente a los ingresos de la película.
...
Como se puede observar en la gráfica la mayoría de votos recibidos fueron en películas cuyo lenguaje original es el ingles.
